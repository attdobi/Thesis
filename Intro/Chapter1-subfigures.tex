...
\caption{BAO peak obtained from BOSS \cite{BOSS}. The correlation function has a peak at of $\rm 100h^{-1}$ Mpc, expected with a dark matter density of $\rm \sim 0.2$.}
\label{fig:BOSS}
\end{figure}
...
In this section we have review current cosmological evidence for the existence of dark matter, and have overviewed the WIMP model and methods of detecting them by scattering off target nuclei.

In Chap.\ 2, we overview the LUX detector, a liquid xenon-time projection chamber (TPC). Concluding the chapter with the most recent LUX science results which holds the worlds leading limit for spin independent WIMP nucleon scattering cross section.

In Chap.\ 3, the position dependent corrections of energy deposition in the LUX detector are discussed.

In Chap.\ 4, the energy scale calibration of the LUX detector is discussed.

In Chap.\ 5 recombination fluctuations in liquid xenon-are overviewed. A method is developed to extract recombination fluctuations from tritium calibration data.

In Chap.\ 6 the results for ionization and scintillation are presented, measured from the tritium calibration source. 
