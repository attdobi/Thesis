...
When Xenon TPCs where first developed it was fully expected that the resolution in the ionization and scintillation channels would be negligible compared to detector resolution.

Let us the 164 keV line from $\rm ^{131}Xe$ used to produce the Doke plots in section \ref{Ch:E_Scale_Cal}. Deconvoling all the components we find a picture illustrated in 
...
$\rm n_\gamma$ & Number of photons & $\rm \left<S1\right>/g1$ \\ \hline
$\rm n_e$ & Number of electrons & $\rm \left<S2\right>/g1$ \\ \hline
...
$\rm \left<r\right>$ & Recombination fraction &  $\rm \left(\left<\frac{n_\gamma}{n_e}\right> - \alpha \right)/\left(\left<\frac{n_\gamma}{n_e}
\right> +1\right)$ \\ \hline
R & Recombined ions &  $\rm \left<r\right>n_{i}$ \\ \hline
$\rm \sigma_{R}$ & Recombination fluctuations &  $\rm \sigma_{\left<r\right>}n_{i}$ \\ \hline
\end{tabular}
\caption{LUX detector parameters, used to measure statistical fluctuations in the light (S1) and charge (S2) channels. Where the S1 and S2 signals have been corrected for position dependance outlined in section \ref{Ch:3}. The values of g1 and g2 where measured in section \ref{Ch:E_Scale_Cal}. }
...
\noindent where $\rm \sigma_{S1_{stat}}^2 $ is the statistical variance of the S1 signal. The variance in the measured number of photons is then equation \ref{eq:Sig_S1} divided by $\rm g_1^2$, using equation \ref{eq:Sig1}.
\begin{equation}
...
\caption{The fluctuations in the S1-x,y,z corrected signal for the 9.4 keV S1 signals from \KrCal. At 9.4 keV the statistical variance is dominate over recombination fluctuations. The resolution is as expected from statistical fluctuations given in equation \ref{eq:Sig_S1}. }
\label{fig:S1_9_Kr}
\end{figure}
...
\noindent where  $\rm \sigma_{n_{\gamma_{inst}}}$ and $\rm \sigma_{n_{e_{inst}}}$ are instrumental fluctuations extracted in section \ref{sec:flucs_mono} that grow like number of quanta n.
