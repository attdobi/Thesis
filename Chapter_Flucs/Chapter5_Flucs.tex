...
\noindent where the gain g1 represents photon detection efficiency, the probability of a photon from an energy deposit striking a PMT and producing a photo electron signal (PE). Gain g2 represents the average S2 signal of a single electron multiplied by the electron extraction efficiency $\rm \epsilon$. Here, we are only using the S2 of the bottom PMT array and is corrected for electron-lifetime. The fluctuations in photons and electrons are related to the observables S1 and S2 by equations \ref{eq:Gain1} and \ref{eq:Gain2}
...
\noindent where $\rm  \sigma_{n_{\gamma_{stat}}}^2$ and $\rm \sigma_{n_{e_{stat}}}^2$ represent the variance in the average number of measured photons and electrons as measured through S1 and S2, respectively. The variances in S1 and S2, $\rm \sigma_{S1_{stat}}^2$ and $\rm \sigma_{S2_{stat}}^2$, are the observable quantities with the detector. The variance in terms of quanta ($\rm n_\gamma$ and $\rm n_e$) must be considered interms of the PE being counted by the PMTs. Note, here we are working with the x,y,z corrected signals outlined in section \ref{Ch:3}. 

The statistical variance of the x,y,z corrected S1 signal, in equation \ref{eq:Sig1}, can be broken into two linearly independent parts. First a binomial variance, for each event there are $\rm n_\gamma$ number of PE to be collected by the PMTs with probability g1. Here, $\rm n_\gamma$ should be thought of as the number of trials.The binomial variance is,
\begin{equation}
\rm \sigma_{S1_{Bino}}^2 = (1-g1) g1 n_\gamma
\label{eq:S1_bino} 
\end{equation}
\noindent where $\rm \sigma_{S1_{Bino}}^2$ is the binomial variance of the S1 light collection process with probability g1 and $\rm n_\gamma$ number of trials. Each PE that was collected by a PMT undergoes a second fluctuation due to the resolution of the PMTs. The variance from PMT resolution can be written as
\begin{equation}
...
The variance in the measured number of photons is then equation \ref{eq:Sig_S1} divided by $\rm g_1^2$, using equation \ref{eq:Sig1}.
\begin{equation}
\rm  \sigma_{n_{\gamma_{stat}}}^2 = \frac{(1-g_1+\sigma_{PE}^2)}{g_1} n_\gamma \\ % change to sigma S1
\label{eq:Sig_Ng}
\end{equation}
...
The variance of the x,y,z corrected S2 signal, given in \ref{eq:Sig2},  is also comprised of several independent processes. Each electron that reaches the liquid-gas interface without being attenuated will either be extracted into the gas producing $\rm SE_b$ number of PE or not. We define $\rm \epsilon$ as the extraction efficiency, or the extraction probability of each electron from the liquid. The binomial variance of such a process is,

\begin{equation}
\rm \sigma_{S2_{bino}}^2= (1-\epsilon) \epsilon n_{e_{z0}} SE_{b}^2 
\label{eq:Sig_S2}
\end{equation}

\noindent where $\rm \epsilon$ is the electron extraction probability, and $\rm n_{e_{z0}}$ is the average number of electrons that reach the liquid surface from the interaction site, also the number of trials. For the case of LUX with an electron lifetime of $\sim 1000 \, \mu s$ and an average drift time of 160 $\rm  \mu s$ the value of $\rm n_{e_{z0}}$ is $\rm \Gamma$ = 0.85. Each electron multiplies to produce $\rm SE_b$ PE in the bottom PMT array, this multiplicative factor must be squared in the variance. 

After extraction, each electron produces $\rm SE_b$ PE with a spread of $\rm \sigma_{SE_{b}}$. The variance from the PMT resolution of a single electron is
\begin{equation}
\rm \sigma_{S2_{PMT}}^2 = \epsilon n_{e_{z0}}  \sigma_{SE}^2
\label{eq:S2_PMT}
\end{equation}

\noindent where $\rm \epsilon n_{e_{z0}}$ is the number of extracted electrons each with PMT resolution $\rm \sigma_{SE}^2$.

For the S2 signal we must also consider the additional variance from electron attenuation, modeled as a Poisson probability of electron capture in each Z slice of the detector. The variance from each Z slice depends of the average number of electrons that will be attenuated. The probability of attenuation at each slice in drift time T is $\rm P(T)= 1-e^{-T/\tau}$, where $\rm \tau$ for the data sets to be considered is 1000 $\rm \mu s$. The drift region considered in the fiducial volume is from 38 to 304.5 $\rm \mu s$. The average variance from events in the fiducial can be given by equation \ref{eq:Sig_Att}.
...
Combining the variances from equations \ref{eq:Sig_S2} \ref{eq:S2_PMT} and \ref{eq:Sig_Att} leads to the the result for the statistical variance in the observed S2 \ref{eq:Sig_S2}.

\begin{equation}
\rm  \sigma_{S2_{stat}}^2 = (1-\epsilon)\epsilon  n_{e_{z0}} SE_b^2 + \epsilon  n_{e_{z0}} \sigma_{SE}^2+  g_2^2 \sigma_{n_{e_{att}}}^2 = (1-\epsilon) SE_b^2 (\Gamma S2) + \sigma_{SE}^2 (\Gamma S2)+  0.155 g_2 S2 \\
\label{eq:Sig_S2}
\end{equation}

\begin{equation}
\rm  \sigma_{n_{e_{stat}}}^2 = \frac{(1-\epsilon)\epsilon SE_b^2 + \epsilon  \sigma_{SE}^2}{g_2^2} \Gamma n_e+  \sigma_{n_{e_{att}}}^2 \\
\label{eq:Sig_Ne}
\end{equation}
