...
Up until this point were have made the assumption that every electron ions pair is a separate entity that either recombines or not. There is no physical justification for the power law fit in figure \ref{fig:R_Big} to the apparent binomial variance amplification. This is indeed a fatal flaw in our recombination model. The observed variance is off by roughly a factor of $\rm N_ion$, surpassing 1000 in the cesium data. There is yet a greater issue with the recombination theory than the failure to describe the variance. The decline of recombination probability as the energy (or $\rm N_i$) tends to zero is unphysical, shown in figure \ref{fig:R_T}. The probability of the electron ion-pair to recombine with its electron pair should be insensitive to the number of electron-ion pairs created by the energy deposit. We take this as a first clue to solving the problem and outline the consequences of allowing for electron-ion pair mixing with some encounter probability in the next two sub-sections. Culminating with a model which will reproduce the observed recombination fraction, explain its decline at low energy, and also naturally yield the correct recombination fluctuations.
...
In the previous subsection a model was introduced which could reproduce the observed recombination fluctuations of ER events. However, the model failed to produce the correct recombination probability. We found that as the energy (or $\rm N_i$) rises the recombination probability tends to 1, and beyond as $\rm r=r_s + N_i\cdot r_\epsilon$. The zeroth order correction to this problem is to remove the self recombination term $\rm r_s$ and treat self recombination as just another encounter recombination probability, $\rm r_s=r_\epsilon$. Next, we will not allow the full $\rm N_i$ electrons to interact with each ion as this is unphysical. Instead, only a small fraction of the total ions $\rm N_\epsilon$ are allowed to have encounters for each energy deposit, illustrated in \ref{fig:Flucs_Fig}. The value of $\rm N_\epsilon$ is set by the requirement to reproduce the correct recombination probability. The observed recombination probability r is then written as,
...
\noindent where $\rm Var_{N_\epsilon r_\epsilon} $ is the recombination variance in number of quanta for a given interaction with $\rm N_i$ ions, encounter recombination probability $\rm r_\epsilon$ with an average of $\rm N_\epsilon$ encounters. The variance derived in equation \ref{eq:SimpVar} grown like $\rm N_i$ and suffers the same problem as our original model. The variance should grow like $\rm N_i^2$ to explain the observer recombination fluctuations in the data, as done in the previous subsection. 

The solution is surprisingly straight forward considering the two requirements. First, to keep the correct observed recombination probability of each interaction the value of  encounter interaction $\rm N_\epsilon$ must be fixed. Second, to get the additional factor of $\rm N_i$ in variance the process of  $\rm N_\epsilon$ should be repeated scaling like $\rm N_i$. Remember, we are only allowing a small number, $\rm N_\epsilon$, of the total ions available ions to have encounters. Consider the interaction picture in figure \ref{fig:Flucs_Fig} for $\rm N_\epsilon$ interaction being repeated $\rm N_i/N_\epsilon$ times, such a process is illustrated in figure \ref{fig:Flucs_Fig_Cluster}.
...
The value of the encounter probability of sub 1\% was also noted by Mozumder in 1995 to explain ion fluctuations in liquid xenon\cite{Mozumder}. 

Recombination fluctuations for ER events have been modeled by a simple binomial process and agree to within 30\% over a wide range of electric fields and energies. The picture of ER band mean and width is now complete, only requiring recombination fraction r as an input. The recombination fraction is dependent on the energy of the interaction and electric field and is described well by modeling from Thomas \& Imel \cite{Thomas_Imel} , Dahl \cite{Dahl_Thesis} and NEST \cite{NEST}, \cite{NEST_2013}. 
