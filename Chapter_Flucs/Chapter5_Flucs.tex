...
\noindent where $\rm Var_{Bino}$ represents the binomial variance, \rp is the recombination probability, and $\rm N_i$ is the number of ions and also the number of trials.
 ...
 Figure \ref{fig:R_Big} shows $\rm \sigma_R$ plotted the number of ions available for recombination $\rm N_i$. The x-axis is chosen to be number of ions as recombination fluctuations only act on ions and not excitons, the conversion to energy on the x axis is $\rm E = W\times n_i (1 + \alpha ) $. The black dashed line in the figure on the right shown the expected standard deviation for a purely binomial process. 

\renewcommand{\baselinestretch}{1}
\small\normalsize
\begin{figure}[h!]\centering
\includegraphics[width=73mm]{Chapter_Flucs/Figures/New_Flucs/R_v_Ni_100_iter1_A.eps}
\includegraphics[width=73mm]{Chapter_Flucs/Figures/New_Flucs/R_v_Ni_100_iter1_C.eps}
\caption{Recombination standard deviation (in quanta) vs. number of ions. The measurements are labeled on the plot and include data from tritium at 170 V/cm, tritium at 100 V/cm, $\rm ^{137}Cs$ calibration, the line sources used for the energy scale calibration listed in table \ref{table:Cal_lines} and a $\rm ^{57}Co$ calibration at several electric fields ranging from 60 to 5000 V/cm from \cite{Dahl_Thesis}. Left: Black dashed lines represent the values expected from a binomial process and the green line is a fit to the data with a constant amplification given in eq ... . Right: a linear fit to the data from equation .}
\label{fig:R_Big}
\end{figure}
\renewcommand{\baselinestretch}{2}
\small\normalsize
